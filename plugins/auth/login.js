            ...
            options: {
                ...
                handler: async (request, h) => {
                    ...
                    if (!config.allowGuestAccess) {
                        return h.response(boom.forbidden('Guest users are not allowed access'));
                    }
            ...
            handler: async (request, h) => {
                try {
                    if (!request.auth.isAuthenticated) {
                        return h.response(
                            boom.unauthorized(`Authentication failed due to: ${request.auth.error.message}`)
                        );
                    }
                    ...
                    if (config.whitelist.length > 0 && !config.whitelist.includes(userDisplayName)) {
                        return h.response(boom.forbidden(`User ${userDisplayName} is not allowed access`));
                    }
