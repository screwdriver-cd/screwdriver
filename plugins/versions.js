...
exports.register = (server, options, next) => {
    ...
    checker.init(
        ...
        (err, json) => {
            if (err) {
                return next(
                    new VError(err, 'Unable to load package dependencies')
                );
            }

            const depArray = Object.keys(json).map(key => ({
                name: key,
                ...json[key]
            }));
            ...
            const sdVersions = depArray
                .filter(dep => SD_REGEX.test(dep.name))
                .map(dep => dep.name);
            ...
            server.route({
                ...
                config: {
                    ...
                    notes:
                        'Returns list of Screwdriver package versions and third-party dependencies',
                    tags: ['api'],
                    response: {
