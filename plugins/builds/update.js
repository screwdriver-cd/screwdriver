...
module.exports = config => ({
    ...
    config: {
        ...
        handler: (request, reply) => {
            ...
            const isBuild =
                scope.includes('build') || scope.includes('temporal');
            const {
                triggerEvent,
                triggerNextJobs
            ...
            if (isBuild && username !== id) {
                return reply(
                    boom.forbidden(`Credential only valid for ${username}`)
                );
            }
            ...
            return buildFactory
                ...
                .then(build => {
                    ...
                    // Check build status
                    if (
                        !['RUNNING', 'QUEUED', 'BLOCKED', 'UNSTABLE'].includes(
                            build.status
                        )
                    ) {
                    ...
                    if (!isBuild) {
                        ...
                        if (adminDetails.isAdmin) {
                            if (
                                desiredStatus !== 'ABORTED' &&
                                desiredStatus !== 'FAILURE'
                            ) {
                                throw boom.badRequest(
                                    'Admin can only update builds ' +
                                        'to ABORTED or FAILURE'
                                );
                            }
                        } else if (desiredStatus !== 'ABORTED') {
                            throw boom.badRequest(
                                'User can only update builds to ABORTED'
                            );
                        }
                ...
                .then(({ build, event }) => {
                    ...
                    switch (desiredStatus) {
                        ...
                        default:
                            throw boom.badRequest(
                                `Cannot update builds to ${desiredStatus}`
                            );
                    }
                    ...
                    if (
                        ['RUNNING', 'COLLAPSED', 'FROZEN'].includes(
                            desiredStatus
                        )
                    ) {
                        return stepFactory
                            ...
                            .then(() =>
                                Promise.all([build.update(), event.update()])
                            );
                    }
                                    ...
                                    return triggerFactory
                                        ...
                                        .then(records => {
                                            ...
                                            records.forEach(record => {
                                                const pipelineId = record.dest.match(
                                                    EXTERNAL_TRIGGER
                                                )[1];

                                                triggeredPipelines.add(
                                                    pipelineId
                                                );
                                            });
