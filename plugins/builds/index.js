...
function getPipelineAndJob(name, pipelineId) {
    ...
    if (isExternalTrigger(name)) {
        [, externalPipelineId, externalJobName] = EXTERNAL_TRIGGER_ALL.exec(
            name
        );
    }
...
function isJoinDone(joinList, finishedBuilds) {
    const successBuilds = finishedBuilds
        ...
        .map(b => b.jobId);
    const successBuildsInJoin = joinList.filter(j =>
        successBuilds.includes(j.id)
    );
...
function noFailureSoFar(joinList, finishedBuilds) {
    const failedBuilds = finishedBuilds
        ...
        .map(b => b.jobId);
    const failedBuildsInJoin = joinList.filter(j =>
        failedBuilds.includes(j.id)
    );
...
function handleNextBuild({ buildConfig, joinList, finishedBuilds, jobId }) {
    return Promise.resolve()
        .then(() => {
            ...
            const successBuildsIds = successBuildsInJoinList(
                joinList,
                finishedBuilds
            ).map(b => b.id);
...
function removeDownstreamBuilds(config) {
    ...
    const visitedBuilds = dfs(
        parentEvent.workflowGraph,
        startFrom,
        builds,
        new Set()
    );
...
function createParentBuildsObj(config) {
    ...
    if (joinListNames) {
        ...
        joinListNames.forEach(name => {
            ...
            joinParentBuilds[joinInfo.externalPipelineId].jobs[
                joinInfo.externalJobName
            ] = null;
        });
 ...
 */
async function getParentBuildStatus({
    newBuild,
    joinListNames,
    pipelineId,
    buildFactory
    ...
    for (let i = 0; i < joinListNames.length; i += 1) {
        ...
        if (
            ...
            bId =
                upstream[joinInfo.externalPipelineId].jobs[
                    joinInfo.externalJobName
                ];
        }
    ...
    joinedBuilds.forEach(b => {
        ...
        if (
            ['FAILURE', 'ABORTED', 'COLLAPSED', 'UNSTABLE'].includes(b.status)
        ) {
            hasFailure = true;
        }
...
async function createOrRunNextBuild({
    ...
    if (isExternal) {
        ...
        if (event.parentEventId) {
            ...
            finishedInternalBuilds = finishedInternalBuilds.concat(
                parallelBuilds
            );
                ...
                Object.keys(parentBuilds[pid].jobs).forEach(jName => {
                    ...
                    if (parentBuilds[pid].jobs[jName] === null) {
                        ...
                        if (parseInt(pid, 10) === pipelineId) {
                            parentJob = workflowGraph.nodes.find(
                        ...
                        if (parentJob) {
                            jobId = parentJob.id;
                            const parentJobBuild = finishedInternalBuilds.find(
                                b => b.jobId === jobId
                            );

                            if (parentJobBuild) {
                                parentBuilds[pid].jobs[jName] =
                                    parentJobBuild.id;
                            } else {
                                logger.warn(
                                    `Job ${jName}:${pid} not found` +
        ...
        const jobId = workflowGraph.nodes.find(
            node => node.name === trimJobName(nextJobName)
        ).id;
...
async function handleDuplicatePipelines(config) {
    ...
    const externalJobPipelineIds = externalJobNamesWithNoJoinArr.map(
        n => EXTERNAL_TRIGGER_ALL.exec(n)[1]
    );
    ...
    const duplicatePipelineIds = Object.keys(uniqPipelineIds).filter(
        a => uniqPipelineIds[a] > 1
    );
...
exports.register = (server, options, next) => {
    ...
    server.expose('triggerNextJobs', async config => {
        ...
        let joinObj = nextJobs.reduce((obj, jobName) => {
            obj[jobName] = workflowParser.getSrcForJoin(workflowGraph, {
                jobName
            });
                    ...
                    return Promise.resolve()
                        ...
                        .then(finishedBuilds =>
                            handleNextBuild({
                                ...
                                joinList,
                                finishedBuilds,
                                jobId: workflowGraph.nodes.find(
                                    node =>
                                        node.name === trimJobName(nextJobName)
                                ).id
                            })
                        );
                })
        ...
        const processNextJob = async nextJobName => {
            ...
            const { externalPipelineId, externalJobName } = getPipelineAndJob(
                nextJobName,
                pipelineId
            );
            ...
            if (joinListNames.length === 0 || currentJobNotInJoinList) {
                ...
                if (
                    build.parentBuilds &&
                    build.parentBuilds[externalPipelineId]
                ) {
                    // TODO: refactor this section to reduce number of DB calls
                    const externalEventId =
                        build.parentBuilds[externalPipelineId].eventId;
                    const externalEvent = await eventFactory.get(
                        externalEventId
                    );
                    const externalPipeline = await pipelineFactory.get(
                    ...
                    const jobId = parentWorkflowGraph.nodes.find(
                        node => node.name === trimJobName(externalJobName)
                    ).id;
                    // Get next build
                    const nextBuild = finishedExternalBuilds.find(
                    ...
                    if (externalEventId) {
                        ...
                        Object.keys(parentBuilds).forEach(pid => {
                            parentBuilds[pid].eventId = event.id;
                            Object.keys(parentBuilds[pid].jobs).forEach(
                                jName => {
                                    let joinJobId;

                                    if (
                                        parentBuilds[pid].jobs[jName] === null
                                    ) {
                                        if (parseInt(pid, 10) === pipelineId) {
                                            joinJobId = workflowGraph.nodes.find(
                                                node =>
                                                    node.name ===
                                                    trimJobName(jName)
                                            ).id;
                                        ...
                                        parentBuilds[pid].jobs[
                                            jName
                                        ] = finishedInternalBuilds.find(
                                            b => b.jobId === joinJobId
                                        ).id;
                                    }
                                }
                    ...
                    if (!nextBuild) {
                        const parentSrc = workflowGraph.edges.find(
                            edge => edge.dest === currentJobName
                        ).src;
                        const parentJobName = getPipelineAndJob(parentSrc)
                            .externalJobName;
                        ...
                        if (previousBuild) {
                            ...
                            newBuild = newEvent.builds.filter(
                                b => b.jobId === jobId
                            )[0];
                        } else {
                            const parentBuildId =
                                build.parentBuilds[externalPipelineId].jobs[
                                    parentJobName
                                ];
                    ...
                    const nextJobsForJoin = workflowParser.getNextJobs(
                        parentWorkflowGraph,
                        {
                    ...
                    const joinObjForJoin = nextJobsForJoin.reduce(
                        (obj, jobName) => {
                            obj[
                                jobName
                            ] = workflowParser.getSrcForJoin(
                                parentWorkflowGraph,
                                { jobName }
                            );
        ...
        for (const nextJobName of nextJobNames) {
            try {
                ...
                logger.error(
                    `Error in processNextJob - pipeline:${pipelineId}-${nextJobName}` +
                        ` event:${event.id} `,
                    err
                );
            }
