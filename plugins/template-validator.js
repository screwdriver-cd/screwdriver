...
const validator = require('screwdriver-template-validator').parseTemplate();
...
const pipelineTemplateValidatorPlugin = {
    ...
    async register(server) {
        server.route({
            ...
            options: {
                description: 'Validate a given sd-pipeline-template.yaml',
                notes: 'returns the parsed config, validation errors, or both',
                tags: ['api', 'validation', 'yaml'],
                plugins: {
                    'hapi-rate-limit': {
...
module.exports = {
    templateValidatorPlugin,
    pipelineTemplateValidatorPlugin
};
