...
describe('pipeline plugin test', () => {
    ...
    let server;
    const password =
        'this_is_a_password_that_needs_to_be_atleast_32_characters';
    ...
    describe('GET /pipelines', () => {
        ...
        it('returns 200 and all pipelines', () => {
            ...
            return server.inject(options).then(reply => {
                ...
                assert.deepEqual(
                    reply.result,
                    testPipelines.concat(gitlabTestPipelines)
                );
            });
        });
    ...
    describe('GET /pipelines/{id}', () => {
        ...
        it('exposes a route for getting a pipeline', () => {
            pipelineFactoryMock.get
                .withArgs(id)
                .resolves(getPipelineMocks(testPipeline));
    ...
    describe('DELETE /pipelines/{id}', () => {
        ...
        it(
            'returns 403 when user does not have admin permission and is not ' +
                'Screwdriver admin',
            () => {
                const error = {
                    statusCode: 403,
                    error: 'Forbidden',
                    message:
                        'User d2lam does not have admin permission for this repo'
                };
                ...
                userFactoryMock.get
                    .withArgs({ username: 'd2lam', scmContext })
                    .resolves(userMock);
                userMock.getPermissions
                    .withArgs(scmUri)
                    .resolves({ admin: false });
    ...
    describe('GET /pipelines/{id}/badge', () => {
        ...
        it('returns 302 to for a valid PR build', () => {
            ...
            return server.inject(`/pipelines/${id}/badge`).then(reply => {
                ...
                assert.deepEqual(
                    reply.headers.location,
                    'foo%2Fbar/1 success, 1 failure/red'
                );
            });
        });
    ...
    describe('GET /pipelines/{id}/{jobName}/badge', () => {
        ...
        it('returns 302 to for a job that is disabled', () => {
            ...
            return server
                .inject(`/pipelines/${id}/${jobName}/badge`)
                .then(reply => {
                    assert.equal(reply.statusCode, 302);
                    assert.deepEqual(
                        reply.headers.location,
    ...
    describe('POST /pipelines/{id}/sync', () => {
        ...
        it('returns 403 when user does not have admin permission', () => {
            const error = {
                ...
                error: 'Forbidden',
                message:
                    'User d2lam does not have push permission for this repo'
            };
    ...
    describe('POST /pipelines/{id}/sync/pullrequests', () => {
        ...
        it('returns 403 when user does not have push permission', () => {
            const error = {
                ...
                error: 'Forbidden',
                message:
                    'User batman does not have push permission for this repo'
            };
    ...
    describe('POST /pipelines', () => {
        ...
        const unformattedCheckoutUrl =
            'git@github.com:screwdriver-cd/data-MODEL.git';
        const formattedCheckoutUrl =
        ...
        it('returns 201 and correct pipeline data', () => {
            ...
            const testDefaultCollection = Object.assign(testCollection, {
                type: 'default'
            });
    ...
    describe('PUT /pipelines/{id}', () => {
        ...
        it('formats the checkout url correctly branch is provided', () => {
            options.payload.checkoutUrl =
                'git@github.com:screwdriver-cd/data-MODEL.git#branchName';
    ...
    describe('PUT /pipelines/{pipelineId}/tokens/{tokenId}', () => {
        ...
        it('returns 409 when the token already exists', () => {
            ...
            pipelineMock.tokens = Promise.resolve(
                getTokenMocks([testTokens, duplicated])
            );
