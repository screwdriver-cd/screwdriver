...
describe('auth plugin test', () => {
    ...
    const jwtPrivateKey = fs
        .readFileSync(`${__dirname}/data/jwt.private.key`)
        .toString();
    const jwtPublicKey = fs
        .readFileSync(`${__dirname}/data/jwt.public.key`)
        .toString();
    ...
    const encryptionPassword =
        'this_is_another_password_that_needs_to_be_atleast_32_characters';
    ...
    describe('profiles', () => {
        ...
        it('adds environment', () => {
            ...
            return newServer
                ...
                .then(() => {
                    const profile = newServer.plugins.auth.generateProfile(
                        'batman',
                        'github:github.com',
                        ['user'],
                        {}
            ...
            it('exists', () =>
                server.inject('/auth/login/guest').then(reply => {
                    assert.equal(
                        reply.statusCode,
                        302,
                        'Login route should be available'
                    );
                    assert.isOk(
                        reply.headers.location.match(/auth\/token/),
                        'Redirects to token'
                    );
                }));
            ...
            it('creates a user tries to close a window', () => {
                ...
                return server.inject(webOptions).then(reply => {
                    assert.equal(
                        reply.statusCode,
                        200,
                        'Login/web route should be available'
                    );
                    assert.equal(
                        reply.result,
        ...
        describe('GET', () => {
            it('exists', () =>
                server.inject('/auth/login/github:github.com').then(reply => {
                    ...
                    assert.isOk(
                        reply.headers.location.match(/github.com/),
    ...
    describe('GET /auth/token', () => {
            ...
            server
                ...
                .then(reply => {
                    ...
                    expect(reply.result.token)
                        ...
                        .with.lengthOf(1);
                    expect(reply.result.token).to.be.a.jwt.and.deep.property(
                        'scope[0]',
                        'user'
                    );
                }));
        ...
        it('returns user signed token given an API access token', () => {
            ...
            server
                ...
                .then(reply => {
                    ...
                    expect(
                        reply.result.token
                    ).to.be.a.jwt.and.have.deep.property('scope[0]', 'user');
                });
        });
        ...
        it('returns pipeline signed token given an API access token', () => {
            ...
            server
                ...
                .then(reply => {
                    ...
                    expect(
                        reply.result.token
                    ).to.be.a.jwt.and.have.deep.property(
                        'scope[0]',
                        'pipeline'
                    );
                });
        ...
        it('fails to issue a jwt given an invalid application auth token', () => {
            ...
            return server
                ...
                .then(reply => {
                    assert.calledWith(tokenFactoryMock.get, {
                        value: 'openSaysMe'
                    });
                    assert.equal(
                        reply.statusCode,
                        401,
                        'Login route should be unavailable'
                    );
                    assert.notOk(
                        reply.result.token,
        ...
        describe('with admins', () => {
            ...
            it('returns admin impersonated build token', () =>
                server
                    .inject({
                        url:
                            '/auth/token/474ee9ee179b0ecf0bc27408079a0b15eda4c99d',
                        credentials: {
                            username: 'batman',
                            scmContext,
                            scope: ['user', 'admin']
                    ...
                    .then(reply => {
                        ...
                        expect(
                            reply.result.token
                        ).to.be.a.jwt.and.deep.property('scope[0]', 'build');
                        expect(
                            reply.result.token
                        ).to.be.a.jwt.and.deep.property(
                            'scope[1]',
                            'impersonated'
                        );
                    }));
    ...
    describe('GET /auth/crumb', () => {
        it('returns 200 with a crumb', () => {
            ...
            sinon
                .stub(server.plugins.crumb, 'generate')
                .callsFake(() => mockReturn);
            ...
            server
                ...
                .then(reply => {
                    assert.notEqual(
                        reply.statusCode,
                        404,
                        'Logout route should be available'
                    );
                }));
            ...
            server
                ...
                .then(reply => {
                    assert.equal(
                        reply.statusCode,
                        200,
                        'Logout route returns wrong status'
                    );
    ...
    describe('GET /auth/contexts', () => {
        beforeEach(() => {
            ...
            scm.getDisplayName
                .withArgs({ scmContext: 'github:github.com' })
                .returns('github');
            scm.getDisplayName
                .withArgs({ scmContext: 'github:mygithub.com' })
                .returns('mygithub');
        });
