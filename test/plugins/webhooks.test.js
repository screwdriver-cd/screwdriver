...
describe('webhooks plugin test', () => {
    ...
    it('registers the plugin', () => {
        ...
        assert.equal(
            server.app.buildFactory.tokenGen('12345'),
    ...
    describe('POST /webhooks', () => {
        ...
        const fullCheckoutUrl =
            'git@github.com:baxterthehacker/public-repo.git#master';
        ...
        it('returns 204 for unsupported event type', () => {
            pipelineFactoryMock.scm.parseHook
                .withArgs(reqHeaders, {})
                .resolves(null);
        ...
        describe('release event', () => {
            ...
            it('returns 204 when getCommitRefSha() is rejected', () => {
                pipelineFactoryMock.scm.getCommitRefSha.rejects(
        ...
        describe('push event', () => {
            ...
            it('returns 201 on success for pipelines with rootDir', () => {
                ...
                pipelineFactoryMock.scm.getChangedFiles.resolves([
                    'lib/test.js'
                ]);
                pipelineFactoryMock.list.resolves([
                    pipelineMock,
            ...
            it('returns 201 when "[skip ci]"', () => {
                ...
                return server.inject(options).then(reply => {
                    assert.calledWith(eventFactoryMock.create, {
                        ...
                        causeMessage: `Merged by ${username}`,
                        skipMessage:
                            'Skipping due to the commit message: [skip ci]',
                        meta: {}
                    });
                    assert.equal(reply.statusCode, 201);
                });
        ...
        describe('pull-request event', () => {
            ...
            describe('open pull request', () => {
                ...
                beforeEach(() => {
                    ...
                    options.payload = testPayloadOpen;
                    scmConfig.prNum = 2;
                    eventFactoryMock.scm.getPrInfo
                        .withArgs(scmConfig)
                        .resolves(prInfo);
                    pipelineFactoryMock.scm.parseHook
                        .withArgs(reqHeaders, options.payload)
                        .resolves(parsed);
                    pipelineFactoryMock.scm.getDisplayName
                        .withArgs({ scmContext })
                    ...
                    expected = {
                        ...
                        changedFiles,
                        configPipelineSha:
                            'a402964c054c610757794d9066c96cee1772daed',
                        pipelineId,
                        prInfo,
                        prNum: 2,
                        prRef,
                ...
                it('returns 201 when getCommitSha() is rejected', () => {
                    pipelineFactoryMock.scm.getCommitSha.rejects(
                        new Error('some error')
                    );
                ...
                it('returns 500 when failed', () => {
                    eventFactoryMock.create.rejects(
                ...
                it('creates empty event if pr from fork by default', () => {
                    ...
                    expected.skipMessage =
                        'Skipping build since pipeline is configured' +
            ...
            describe('synchronize pull request', () => {
                ...
                it('has the workflow for stopping builds before starting a new one', () => {
                    const abortMsg =
                        'Aborted because new commit was pushed to PR#1';
                    ...
                    return server.inject(options).then(reply => {
                        ...
                        assert.calledWith(eventFactoryMock.create, {
                            ...
                            changedFiles,
                            causeMessage:
                                'Synchronized by github:baxterthehacker',
                            chainPR: false,
                            baseBranch: 'master'
                        });
                        assert.isOk(
                            model1.update.calledBefore(eventFactoryMock.create)
                        );
                        assert.isOk(
                            model2.update.calledBefore(eventFactoryMock.create)
                        );
                ...
                it('creates empty event if pr from fork by default', () => {
                    ...
                    parsed.prSource = 'fork';
                    expected.skipMessage =
                        'Skipping build since pipeline is ' +
            ...
            describe('close pull request', () => {
                ...
                it('stops running builds', () =>
                    server.inject(options).then(() => {
                        ...
                        assert.strictEqual(model1.status, 'ABORTED');
                        assert.strictEqual(
                            model1.statusMessage,
                            'Aborted because PR#1 was closed'
                        );
                        assert.strictEqual(model2.status, 'ABORTED');
                        assert.strictEqual(
                            model2.statusMessage,
                            'Aborted because PR#1 was closed'
                        );
                    }));
        ...
        describe('something went wrong with parseHook', () => {
            it('returns 500 when failed', () => {
                pipelineFactoryMock.scm.parseHook.rejects(
                    new Error('Invalid x-hub-signature')
                );
