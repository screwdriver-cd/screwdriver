            ...
            server.register(
                {
                    ...
                    options: {
                        terminationGracePeriod:
                            parseInt(
                                process.env.TERMINATION_GRACE_PERIOD,
                                10
                            ) || 30
                    }
                },
...
module.exports = (server, config) =>
    new Promise((resolve, reject) => {
        ...
        return Object.keys(notificationConfig).forEach(plugin => {
            const Plugin = requireNotificationPlugin(
                notificationConfig[plugin],
                plugin
            );
            let notificationPlugin;

            if (notificationConfig[plugin].config) {
                notificationPlugin = new Plugin(
                    notificationConfig[plugin].config
                );
            } else {
            ...
            notificationPlugin.events.forEach(event => {
                server.on(event, buildData =>
                    notificationPlugin.notify(buildData)
                );
            });
        });
    });
