...
module.exports = async config => {
    try {
        ...
        server.ext('onCredentials', (request, h) => {
            const { username, scmContext, pipelineId } = request.auth.credentials;

            if (pipelineId) {
                logger.info(`[Login] Pipeline ${pipelineId}:${scmContext} ${request.method} ${request.path}`);
            } else {
                logger.info(`[Login] User ${username}:${scmContext} ${request.method} ${request.path}`);
            }
