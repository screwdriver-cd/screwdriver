 ...
 */
function prettyPrintErrors(request, h, release) {
    const { response } = request;

    if (release && release.mode === 'canary' && !(request.state && request.state[release.cookieName])) {
...
module.exports = async config => {
    try {
        ...
        const canSetCookie = config.release && config.release.mode === 'canary';

        if (canSetCookie) {
            server.state(config.release.cookieName, {
                path: '/',
                ttl: config.auth.sessionTimeout * 60 * 1000,
        ...
        server.ext('onPreResponse', (request, h) => {
            return prettyPrintErrors(request, h, config.release);
        });
