...
Feature: Pipeline Token
  ...
  Scenario: Generate New Pipeline Token
    Given "calvin" dose not own a token named "tiger"
  ...
  Scenario: List Pipeline Tokens
    Given "calvin" owns an existing Pipeline token named "tiger"
  ...
  Scenario: Call API using a invalid pipeline token
    Given "calvin" owns an invalid Pipeline token named "tiger"
