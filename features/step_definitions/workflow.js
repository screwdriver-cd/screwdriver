            ...
            this.getJwt(this.apiToken)
                ...
                .then(response => {
                    ...
                    if (response.statusCode === 201) {
                        ...
                        this.pipelineId = response.body.message.split(
                            /\s*:\s*/
                        )[1];
                    }
    ...
    function step(triggeredJobName, parentJobName) {
        return sdapi
            ...
            .then(builds => {
                ...
                const parentBuild = this.builds.find(
                    b => b.jobId === parentJob.id
                );
                const triggeredJob = this.jobs.find(
                    j => j.name === triggeredJobName
                );
                const triggeredBuild = this.builds.find(
                    b => b.jobId === triggeredJob.id
                );
    ...
    function step(jobName) {
        return this.waitForBuild(this.buildId).then(resp => {
            ...
            Assert.equal(
                resp.body.status,
                'SUCCESS',
                `Unexpected build status: ${jobName}`
            );
        });
    }
);
